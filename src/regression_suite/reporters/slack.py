...
class SlackReporter(ReporterPort):
    def __init__(self, webhook_url: str, channel: str = "#regression", post_on_success: bool = False) -> None:
        self._url, self._channel, self._on_success = webhook_url, channel, post_on_success
    ...
    def report(self, sr: SuiteResult) -> None:
        ...
        payload = {"channel": self._channel, "attachments": [{"color": color,
            "title": f"{emoji} Regression {'PASSED' if sr.all_passed else 'FAILED'}: {sr.suite_name}",
            "fields": [{"title": "Passed", "value": str(sr.passed_count), "short": True},
