...
class EndToEndDataFlowCheck(CheckPort):
    ...
    @property
    def name(self) -> str:
        return "service.end_to_end_flow"
    ...
    def run(self) -> CheckResult:
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                row = cur.fetchone()
                assert row and row[0] == marker
                cur.execute(
                    "SELECT COUNT(*), AVG(value) FROM sensor_readings WHERE sensor_id = %s",
                    (marker,),
                )
                agg = cur.fetchone()
                assert agg[0] >= 1
                cur.execute("DELETE FROM sensor_readings WHERE sensor_id = %s", (marker,))
                conn.commit()
            conn.close()
            ...
            return CheckResult(
                ...
                message=f"Insert/query/aggregate/cleanup in {elapsed:.0f}ms",
                duration_ms=elapsed,
