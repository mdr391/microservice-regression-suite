...
class PostgresConnectCheck(CheckPort):
    def __init__(self, dsn: str) -> None:
        self._dsn = dsn
    @property
    def name(self) -> str: return "postgres.connectivity"
    @property
    def suite(self) -> str: return "postgres"
    @property
    def severity(self) -> CheckSeverity: return CheckSeverity.CRITICAL
    @property
    def timeout_seconds(self) -> float: return 5.0
    ...
    def run(self) -> CheckResult:
        ...
        try:
            ...
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.PASS,
                               message=f"Connected in {elapsed:.0f}ms", duration_ms=elapsed, severity=self.severity)
        except ImportError:
            ...
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.FAIL,
                               message=f"Connection failed: {e}", duration_ms=(time.perf_counter()-start)*1000,
...
class PostgresReadWriteCheck(CheckPort):
    ...
    def __init__(self, dsn: str) -> None:
        self._dsn = dsn
    @property
    def name(self) -> str: return "postgres.read_write"
    @property
    def suite(self) -> str: return "postgres"
    @property
    def severity(self) -> CheckSeverity: return CheckSeverity.CRITICAL
    ...
    def run(self) -> CheckResult:
        ...
        try:
            ...
            conn = psycopg2.connect(self._dsn, connect_timeout=3)
            with conn.cursor() as cur:
                cur.execute("INSERT INTO sensor_readings (sensor_id, value, recorded_at) VALUES (%s, %s, NOW()) RETURNING reading_id", (test_id, 99.99))
                row_id = cur.fetchone()[0]
                conn.commit()
                cur.execute("SELECT sensor_id, value FROM sensor_readings WHERE reading_id = %s", (row_id,))
                row = cur.fetchone()
            ...
            elapsed = (time.perf_counter() - start) * 1000
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.PASS,
                               message=f"Write/read/verify/cleanup in {elapsed:.0f}ms", duration_ms=elapsed, severity=self.severity)
        except ImportError:
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.SKIP,
                               message="psycopg2 not installed", duration_ms=(time.perf_counter()-start)*1000, severity=self.severity)
        except Exception as e:
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.FAIL,
                               message=f"Read/write failed: {e}", duration_ms=(time.perf_counter()-start)*1000,
                               severity=self.severity, error=str(e))


class PostgresSchemaCheck(CheckPort):
    """Verify expected tables and columns exist."""
    EXPECTED = {"sensor_readings": ["reading_id", "sensor_id", "value", "recorded_at"],
                "sensors": ["sensor_id", "location", "unit", "active"]}

    def __init__(self, dsn: str) -> None:
        self._dsn = dsn
    @property
    def name(self) -> str: return "postgres.schema_integrity"
    @property
    def suite(self) -> str: return "postgres"
    @property
        ...
        try:
            ...
            with conn.cursor() as cur:
                for table, cols in self.EXPECTED.items():
                    cur.execute("SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name=%s AND table_schema='public')", (table,))
                    if not cur.fetchone()[0]:
                        missing_tables.append(table); continue
            ...
            if missing_tables or missing_cols:
                return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.FAIL,
                                   message="Schema drift detected", duration_ms=elapsed, severity=self.severity,
                                   details={"missing_tables": missing_tables, "missing_columns": missing_cols})
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.PASS,
                               message=f"All {len(self.EXPECTED)} tables verified", duration_ms=elapsed, severity=self.severity)
        except ImportError:
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.SKIP,
                               message="psycopg2 not installed", duration_ms=(time.perf_counter()-start)*1000, severity=self.severity)
        except Exception as e:
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.FAIL,
                               message=f"Schema check failed: {e}", duration_ms=(time.perf_counter()-start)*1000,
                               severity=self.severity, error=str(e))
...
class PostgresQueryPerformanceCheck(CheckPort):
    ...
    def __init__(self, dsn: str, max_latency_ms: float = 500.0) -> None:
        self._dsn, self._max = dsn, max_latency_ms
    @property
    def name(self) -> str: return "postgres.query_performance"
    @property
    def suite(self) -> str: return "postgres"
    ...
    def run(self) -> CheckResult:
        ...
        try:
            ...
            with conn.cursor() as cur:
                t = time.perf_counter()
                cur.execute("SELECT sensor_id, COUNT(*), AVG(value) FROM sensor_readings WHERE recorded_at >= NOW() - INTERVAL '24 hours' GROUP BY sensor_id ORDER BY COUNT(*) DESC LIMIT 10")
                cur.fetchall()
            ...
            return CheckResult(name=self.name, suite=self.suite, status=CheckStatus.FAIL,
                               message=f"Performance check failed: {e}", duration_ms=(time.perf_counter()-start)*1000,
                               severity=self.severity, error=str(e))
