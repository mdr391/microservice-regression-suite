...
class DemoCheck(CheckPort):
    ...
    def __init__(self, check_name: str, check_suite: str, sev: CheckSeverity = CheckSeverity.MEDIUM,
                 status: CheckStatus = CheckStatus.PASS, latency: float = 50.0, msg: str = "OK") -> None:
        self._n, self._s, self._sev, self._st, self._lat, self._msg = check_name, check_suite, sev, status, latency, msg
    @property
    def name(self) -> str: return self._n
    @property
    def suite(self) -> str: return self._s
    @property
    def severity(self) -> CheckSeverity: return self._sev
    def run(self) -> CheckResult:
        time.sleep(self._lat / 1000)
        return CheckResult(name=self.name, suite=self.suite, status=self._st,
                           message=self._msg, duration_ms=self._lat, severity=self.severity)


def _demo_checks() -> list[CheckPort]:
    S = CheckStatus; V = CheckSeverity
    return [
        DemoCheck("grpc.connectivity", "grpc", V.CRITICAL, S.PASS, 45, "Channel ready at localhost:50051"),
        DemoCheck("grpc.health", "grpc", V.CRITICAL, S.PASS, 32, "Service SERVING"),
        DemoCheck("grpc.health.SensorService", "grpc", V.CRITICAL, S.PASS, 28, "SensorService SERVING"),
        DemoCheck("grpc.latency", "grpc", V.MEDIUM, S.PASS, 120, "Avg latency 24ms (SLO: 200ms)"),
        DemoCheck("postgres.connectivity", "postgres", V.CRITICAL, S.PASS, 18, "Connected in 18ms"),
        DemoCheck("postgres.read_write", "postgres", V.CRITICAL, S.PASS, 65, "Write/read/verify/cleanup in 65ms"),
        DemoCheck("postgres.schema_integrity", "postgres", V.HIGH, S.PASS, 42, "All tables verified"),
        DemoCheck("postgres.query_performance", "postgres", V.MEDIUM, S.PASS, 88, "Query 88ms (SLO: 500ms)"),
        DemoCheck("service.end_to_end_flow", "service", V.HIGH, S.PASS, 150, "Insert/query/aggregate/cleanup in 150ms"),
        DemoCheck("health.http_endpoint", "health", V.CRITICAL, S.PASS, 22, "Health returned 200 in 22ms"),
        DemoCheck("grpc.latency.SensorService", "grpc", V.MEDIUM, S.FAIL, 350, "Avg latency 250ms exceeds SLO 200ms"),
    ]
...
def _register_live_checks(runner: RegressionRunner, config: dict) -> None:
    ...
    from .checks.postgres import PostgresConnectCheck, PostgresReadWriteCheck, PostgresSchemaCheck, PostgresQueryPerformanceCheck
    from .checks.service import EndToEndDataFlowCheck, HttpHealthEndpointCheck

    g = config.get("grpc", {}); p = config.get("postgres", {}); s = config.get("service", {})
...
def main() -> None:
    ...
    parser.add_argument("--demo", action="store_true", help="Run with mock checks (no services needed)")
    ...
    if not args.demo:
        ...
        if url := rpt.get("slack_webhook"):
            reporters.append(SlackReporter(url, rpt.get("slack_channel", "#regression"), rpt.get("slack_on_success", False)))
        if prom := rpt.get("prometheus", {}):
            if prom.get("enabled"):
                from .reporters.prometheus import PrometheusReporter
                reporters.append(PrometheusReporter(prom["pushgateway_url"], prom.get("job_name", "regression"), prom.get("environment", "staging")))
